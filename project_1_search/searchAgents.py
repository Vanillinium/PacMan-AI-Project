...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    walls = problem.walls
    current_pos, visited_tuple = state
    unvisited_corners = []

    for i in range(len(corners)):
        if not visited_tuple[i]:
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    "Case 1: Mahattan Distance - 9928 nodes"
    # costs = []
    ...
    "Case 2: Distance between 2 foods - 9242 nodes"
    # costs = []
    ...
    "Case 3: Closet food to Pacman + Distance between 2 foods - 7856 nodes"
    if len(foodList) == 1:
        food = foodList[0]
        return abs(position[0] - food[0]) + abs(position[1] - food[1])
    
    minDistanceFromPacman = 9999
    for food in foodList:
        distance = abs(position[0] - food[0]) + abs(position[1] - food[1])

        minDistanceFromPacman = min(distance, minDistanceFromPacman)
    ...
    for i in range(len(foodList)):
        for j in range(i + 1, len(foodList)):
            ...
            distance = abs(food1[0] - food2[0]) + abs(food1[1] - food2[1])
            maxDistanceApart = max(distance, maxDistanceApart)

    return minDistanceFromPacman + maxDistanceApart
